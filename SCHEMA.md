...
> "저는 이 모양보다 꼬냑 비세토스 패턴이 더 중요해요. 형태가 조금 달라도 직접 보고 싶어요."

essential 은 비세토스 패턴/컬러 쪽으로 좁게 잡히고 physicalCheckAttributes 로 실물 확인을 요청한다. 형태(SKU 5, 9 등)가 아니라 패턴·컬러가 일치하는 SKU 3 이 선택된다.
...
> **이슈 #33**: 데모 시나리오 A(정확한 제품 우선)도 이 경로를 사용한다. 기존 시나리오 A 발화
> ("이 로고 위치와 각진 형태가 좋아요")는 `{logoPosition:정면중앙, silhouette:사각}` 을 추출하는데,
> 이 조합은 SKU 3·4·5·9·11 도 함께 만족하므로 원제품에만 유일하게 성립하지 않는다. 그 결과 매장 내
> 후보가 ③-보조 폴백으로 먼저 선택되어 `EXACT_PRODUCT` 에 도달할 수 없었다. **`EXACT_PRODUCT` 는
> 구조상 "원제품에만 유일하게 성립하는 필수 조건"이 있어야 나오는 결과**이므로, 발화가 원제품 고유
> 속성(`handleType=다이아몬드컷아웃` + `material=캔버스`)을 담도록 조정했다.
> 시나리오 A 확정 발화: "손잡이의 다이아몬드 컷아웃 디자인과 비세토스 캔버스 소재는 꼭 그대로였으면
> 해요. 오늘 당장 필요하진 않아요"
